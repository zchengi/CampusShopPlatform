...
$(function () {
    ...
    var loading = false,
        ...
        pageNum = 1,
        // 获取该用户的奖品领取记录列表的 url
        listUrl = '/o2o/frontend/listuserproductmapsbycustomer',
        productName = '',
        $searchBar = $('#searchBar'),
            ...
            if (data.success) {
                ...
                data.userProductMapList.map(function (item) {
                    ...
                    productListHtml += '<div class="weui-cells" data-product-id="' + item.productId + '">' +
                        ...
                        '</div>' +
                        '<div class="weui-cell">' +
                        '<div class="weui-cell__bd my-cell_bd">' + item.product.productName + '</div>' +
                        '</div>' +
                        '<div class="weui-cell">' +
                        '<div class="weui-cell__bd">' + new Date(item.createTime).Format('yyyy-MM-dd') + '</div>' +
                        '<div class="weui-cell__ft">积分：' + item.point + '</div>' +
                        '</div></div>';
                });
